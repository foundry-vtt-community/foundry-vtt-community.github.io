...
* Explode with "operator" does not really need a number after operator and maximum die value is used in that case. Example: `/roll 1d4x=` will explode on 4.* Inline rolls can be deferred by adding slash-command after opening brackets. Deferred rolls must be clicked in chat to show the result. Example: `Make either dexterity based roll [[/roll 1d20 + @abilities.dex.mod ]] or str based roll [[/roll 1d20 + @abilities.str.mod ]]`
